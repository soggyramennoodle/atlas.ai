...
| 6 | Admin (7 pages + admin/newsroom components) | DONE | bb78c05e |
| 7 | Final sweep (theme files, rivo tokens, dead styles) | DONE | (this commit) |

## ✅ ALL PHASES COMPLETE

**2026-06-12 Phase 7 DONE (final sweep):** Theme system fully retired — deleted `theme-provider.tsx`, `theme-selector.tsx`, `theme-sync.tsx`; removed `ThemeProvider` from the root layout; `next-themes` uninstalled; sonner Toaster hardcoded `theme="light"` (its cinematic-surface observer also had a latent bug — it never wrote state on mutation — fixed alongside the lint-required microtask defer). Deleted 8 unused shadcn components (badge, button, card, input, label, separator, skeleton, textarea — avatar/dropdown-menu/sonner/ai-glow remain in use). Removed the dead `.dark { … }` rivo token twin and `.dark .uni-logo` rules from globals.css (`:root` rivo tokens KEPT — shadcn primitives still consume them on remaining surfaces; `@custom-variant dark` kept so the inert `dark:` classes in dropdown-menu still compile). `.next` wiped after the globals.css edit. Verified: eslint clean, tsc clean, 95/95 tests, marketing routes all 200 (/, /login, /signup, /newsroom, /pricing), app routes 307. Remaining known follow-ups: exporters emit raw `$...$` LaTeX (PDF/docx math), and nothing in this redesign has been **visually verified in a browser** — recommend a human pass over dashboard, notes, settings, admin.
...
**2026-06-12 Phase 5 STARTED:** Onboarding + UI tour + passkey prompt restyle is active. Scope: `src/app/onboarding/page.tsx`, `src/components/onboarding/onboarding-flow.tsx`, `src/components/onboarding/ui-tour.tsx`, `src/components/onboarding/spotlight-overlay.tsx`, `src/components/auth/passkey-enrollment-prompt.tsx`, and likely `src/lib/ui-tour-steps.ts` if tour copy/placement needs minor alignment. Contract: onboarding moves off old MarketingBackground into plain `#fafafa` / HeroBand cinematic-light treatment, tour bubbles become ink glass, passkey prompt converts to the same pill/hairline system. Preserve onboarding submit/profile behavior, tour completion/skip/reset behavior, passkey enrollment dismiss/remind/add flows, and every `data-tour` contract.

**2026-06-12 Phase 5 implementation checkpoint:** Converted onboarding to the cinematic-light first-run screen (`src/app/onboarding/layout.tsx` + `src/components/onboarding/onboarding-flow.tsx`), converted the tour spotlight popover to ink glass (`src/components/onboarding/spotlight-overlay.tsx`), removed the stale tour step for the retired theme selector (`src/lib/ui-tour-steps.ts`), and restyled the passkey enrollment prompt (`src/components/auth/passkey-enrollment-prompt.tsx`) with the shared pill/hairline system. Remaining before DONE: run targeted lint, typecheck, tests, blocked-token grep, browser QA for onboarding/tour/passkey eligibility where possible, then final diff review and commit/push.
